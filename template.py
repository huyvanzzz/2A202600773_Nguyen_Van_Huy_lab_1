...
TEST_MODULE_ALIAS = "day01_lab_assignment_template"
sys.modules.setdefault(TEST_MODULE_ALIAS, sys.modules[__name__])
...
def call_openai(
    ...
    latency = max(time.time() - start_time, 1e-9)
...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_cost_estimate = (
        estimated_output_tokens / 1000 * COST_PER_1K_OUTPUT_TOKENS[OPENAI_MODEL]
    )
...
compare_models.__module__ = TEST_MODULE_ALIAS
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    def truncate(text: object, max_length: int = 40) -> str:
        text = str(text)
